...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        center = find_closest(restaurant_location(restaurant),centroids)
        lst.append([center, restaurant])
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    locations = [restaurant_location(res) for res in cluster]
    x, y = zip(*locations)
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    best = max(feature_fns, key=lambda x: find_predictor(user, reviewed, x)[1])
    return find_predictor(user, reviewed, best)[0]
    # END Question 8
