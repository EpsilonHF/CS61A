...
def prune_leaves(t, vals):
    ...
    root = label(t)
    branch = [prune_leaves(b, vals) for b in branches(t)]
    branch = [x for x in branch if x is not None]
...
def add_trees(t1, t2):
    ...
    branch = [add_trees(x, y) for x, y in zip(branches(t1), branches(t2))]

    diff = len(branches(t1)) - len(branches(t2))
    if diff >  0:
        branch += branches(t1)[-diff:]
    if diff < 0:
        branch += branches(t2)[diff:]
