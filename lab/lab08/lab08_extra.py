...
def make_advanced_counter_maker():
    ...
    def new():
        record = {}
        def count(string):
            nonlocal record, global_record
            if string == 'reset':
                record = {}
            elif string == 'global-reset':
                global_record = {}
            elif string.startswith("global"):
                global_record[string] = global_record.get(string, 0) + 1
                return global_record[string]
            else:
                record[string] = record.get(string, 0) + 1
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def transfer(link):
        ...
        while link != Link.empty:
            string += front+str(link.first)+mid
            i += 1
            link = link.rest
        string += empty_repr+back*i
        return string
...
def long_paths(tree, n):
    ...
    def paths(t, pre, i):
        if t.is_leaf() and i >= n:
            res.append(transfer(pre+[t.label]))
        else:
            for b in t.branches:
                paths(b, pre+[t.label], i+1)
